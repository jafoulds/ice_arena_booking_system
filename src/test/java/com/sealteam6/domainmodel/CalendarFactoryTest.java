...
public class CalendarFactoryTest {
    ...
    @Test
    public void bookingStartsAfterAndEndsBeforePreviousBookingTest() throws Exception {
        List<Rink> rinks = rinkRepository.findAll();
        YearMonth yearMonth = YearMonth.of(2000, 1);
        LocalDate date = yearMonth.atDay(1);
        LocalTime openingTime = arenaScheduleService.getOpeningTime(date);
        LocalTime closingTime = arenaScheduleService.getClosingTime(date);
        List<Booking> bookings = new ArrayList<>();
        LocalDateTime startDate1 = LocalDateTime.of(date, openingTime);
        LocalDateTime endDate1 = LocalDateTime.of(date, openingTime.plusHours(3));
        LocalDateTime startDate2 = LocalDateTime.of(date, openingTime.plusHours(1));
        LocalDateTime endDate2 = LocalDateTime.of(date, openingTime.plusHours(2));
        LocalDateTime startDate3 = LocalDateTime.of(date, openingTime);
        LocalDateTime endDate3 = LocalDateTime.of(date, closingTime);
        bookings.add(new Booking(startDate1, endDate1, rinks.get(0), USERNAME, GROUPNAME));
        bookings.add(new Booking(startDate2, endDate2, rinks.get(0), USERNAME, GROUPNAME));
        bookings.add(new Booking(startDate3, endDate3, rinks.get(1), USERNAME, GROUPNAME));
        when(bookingRepository.findByDateBetween(any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(bookings);
        ...
        TimeSlot timeSlot = firstDay.getAvailableTimeSlots().get(0);
        assertThat(timeSlot.getStartTime(), is(openingTime.plusHours(3)));
        assertThat(timeSlot.getEndTime(), is(closingTime));

    }

    @Test
    public void bookingStartsAfterAndEndsAfterPreviousBookingTest() throws Exception {
        List<Rink> rinks = rinkRepository.findAll();
        YearMonth yearMonth = YearMonth.of(2000, 1);
        LocalDate date = yearMonth.atDay(1);
        LocalTime openingTime = arenaScheduleService.getOpeningTime(date);
        ...
        List<Booking> bookings = new ArrayList<>();
        LocalDateTime startDate1 = LocalDateTime.of(date, openingTime);
        LocalDateTime endDate1 = LocalDateTime.of(date, openingTime.plusHours(2));
        LocalDateTime startDate2 = LocalDateTime.of(date, openingTime.plusHours(1));
        LocalDateTime endDate2 = LocalDateTime.of(date, openingTime.plusHours(3));
        LocalDateTime startDate3 = LocalDateTime.of(date, openingTime);
        LocalDateTime endDate3 = LocalDateTime.of(date, closingTime);
        bookings.add(new Booking(startDate1, endDate1, rinks.get(0), USERNAME, GROUPNAME));
        bookings.add(new Booking(startDate2, endDate2, rinks.get(0), USERNAME, GROUPNAME));
